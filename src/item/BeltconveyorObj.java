...
public class BeltconveyorObj extends ObjEX implements java.io.Serializable {
    ...
    public static final int hitCheckObjType = ObjEX.YUKKURI + ObjEX.SHIT + ObjEX.FOOD + ObjEX.TOY + ObjEX.VOMIT + ObjEX.STALK;
...
//    private src.Obj.Type targetType2;
    ...
    {
        for(int i = 0; i < 10; i++)
            images[i] = ModLoader.loadItemImage(loader, (new StringBuilder("beltconveyor/beltconveyor")).append(String.format("%03d", new Object[] {
                Integer.valueOf(i + 1)
            })).append(".png").toString());
	...
	public int getImageLayer(Graphics2D g2, BufferedImage[] layer, Rectangle rect) {
		...
		Translate.getPolygonPoint(anPointBaseX[0], anPointBaseY[0], anPointBaseX[1], anPointBaseY[1], anPointX, anPointY);
		//Translate.getPolygonPoint(fieldSX, fieldSY, fieldEX, fieldEY, anPointX, anPointY);
		TexturePaint texture  = new TexturePaint(layer[0], new Rectangle2D.Float(0, 0, layer[0].getWidth()-10, layer[0].getHeight()-10));
		g2.setPaint(texture);
	...
	public boolean checkHitObj(Rectangle colRect, Obj o){
		if((o instanceof Body) && ((Body)o).isLockmove()) return false;
		if(o.isRemoved()){
	...
	public int objHitProcess(Obj o){
	 	...
	 	// ゆっくりタイプチェック
			...
			if( obOptionSelectionList != null && obOptionSelectionList.size() == 9 ){
				...
				switch( bodyTarget.getAttitude() ){
					case VERY_NICE:
						if( !obOptionSelectionList.get(0) ) return 0;
						break;
					case NICE:
						if( !obOptionSelectionList.get(1)) return 0;
						break;
					case AVERAGE:
						if( !obOptionSelectionList.get(2)) return 0;
						break;
					case SHITHEAD:
						if( !obOptionSelectionList.get(3)) return 0;
						break;
					case SUPER_SHITHEAD:
						if( !obOptionSelectionList.get(4)) return 0;
						break;
				}
				// 知性
				switch(bodyTarget.getIntelligence()) {
					case WISE:
						if( !obOptionSelectionList.get(5)) return 0;
						break;
					case AVERAGE:
						if( !obOptionSelectionList.get(6)) return 0;
						break;
					case FOOL:
						if( !obOptionSelectionList.get(7)) return 0;
						break;
				}
				// 死体のみチェック
		...
		if(targetType != 0){
			switch(targetType){
				case 1:
					if(!(o instanceof Body)) return 0;
					break;
				case 2:
					if(!(o instanceof Shit || o instanceof Vomit)) return 0;
					break;
				case 3:
					if(!(o instanceof Food) || o instanceof Shit || o instanceof Vomit || o instanceof Body) return 0;
					break;
				case 4:
					if(!(o instanceof Stalk)) return 0;
					break;
				default:
					if(o instanceof Body) return 0;
					break;
		...
		if(o instanceof Body) attr = Barrier.MAP_BODY[((Body)o).getBodyAgeState().ordinal()];
			...
			if( bMoveOnce ==true ){
				if( (bindObjList != null) && bindObjList.contains(o) ==true ){
					for( Obj oBind: bindObjList){
						...
						if(!Barrier.onBarrier(oBind.getX(), oBind.getY(), oBind.getW() >> 1, oBind.getH() >> 2, attrBind)){
							bMove = false;
							break ;
						}
					}
...
//			belt.targetType2 = src.Obj.Type.VOMIT;
	...
	//----------------------
		if( !init){
			...
			int[] anPointBaseX = new int[2];
			int[] anPointBaseY = new int[2];
			Translate.getMovedPoint(belt.fieldSX, belt.fieldSY, belt.fieldEX, belt.fieldEY, 0, 0, 0, 0, anPointBaseX, anPointBaseY );
    ...
    public static class ButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			case YUKKURI_FILTER:
				...
				boolean bFilter = YukkuriFilterPanel.openFilterPanel("対象設定", "上段:ベルトコンベアで流さない種類を選択、下段:流す性質を選択", istrOptionList, arrayTemp, obOptionSelectionList );
				if( bFilter  ){
