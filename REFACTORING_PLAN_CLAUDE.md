...
### 3-A. `GameWorld.get().getCurrentWorldState()` の散在を解消する（進行中）
...
段階的アプローチ: `logic/` クラスに `WorldState ws` パラメータを追加し backward-compat overload を提供。
...
REFACTORING_PLAN_PACKAGE.md の優先度 A は完了済み。

1. `draw` → `engine` への移動: ✅ 完了
   `GameLoop`, `ImageLoadService`, `ModLoader`, `SaveDataCodec`,
   `World`, `YukkuriFactory`, `YukkuriTickProcessor`

2. `system` → `ui` への移動: ✅ 完了
   `ItemWindow`, `LoadWindow`, `MainCommandUI`,
   `MouseInputController`, `ShowStatusFrame`

3. 優先度 B: `system.Sprite` / `system.YukkuriLayer` の配置再検討（任意）
...
- `ImageCode.EXCITING_raper` → `EXCITING_RAPER` ✅
- `GadgetMenu.GadgetList` → `GadgetMenuChoice` ✅
- `MessageMap` → 削除済み ✅
- `YukkuriFamilyDelegate.checkRemovedFamilyList()` → 改名済み ✅
