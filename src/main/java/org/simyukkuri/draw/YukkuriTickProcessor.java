...
public final class YukkuriTickProcessor {
	 ...
	 */
	public static Yukkuri processYukkuri(Terrarium terrarium, MapPlaceData curMap, Yukkuri yukkuri, List<Yukkuri> babyList,
			boolean transCheck) {
		Event ret = yukkuri.clockTick();
		switch (ret) {
			case DEAD:
		...
		if (yukkuri.getPanicType() != null && !yukkuri.isUnBirth() && !yukkuri.isDamagedHeavily()) {
			...
			List<Yukkuri> childrenList = YukkuriLogic.createActiveChildList(yukkuri, true);
