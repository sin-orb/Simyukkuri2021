 ...
 * <p>
 * Phase 2では、まず{@link Yukkuri#moveYukkuri(boolean)}全体を移すのではなく、
 * 移動速度に関する判定だけをこのクラスへ委譲する。落下、衝突、目的地更新などの
 * 副作用はまだ{@link Yukkuri}側に残し、挙動変更の範囲を小さく保つ。
 * </p>
 */
public final class YukkuriMovement {
	private YukkuriMovement() {
	}
	 ...
	 * 家族イベント中の最低速度を既存の{@link Yukkuri#moveYukkuri(boolean)}と同じ順序で反映する。
	 * </p>
	 *
	...
	public static int calculateMovementStep(Yukkuri body) {
		int step = body.getStepBase()[body.getAgeState().ordinal()];
	 ...
	 * <p>
	 * 目的地へ到達済みの場合は、既存の{@link Yukkuri#moveYukkuri(boolean)}と同じく
	 ...
	 * @param toZ    Z座標
	 */
	public static void moveToYukkuri(Yukkuri body, Entity target, int toX, int toY, int toZ) {
		body.clearActions();
	 ...
	 * <p>
	 * 既存の{@link Yukkuri#moveYukkuri(boolean)}先頭にあった物理更新をそのまま移したもの。
	...
	private static int getRandomDirectionLimit(Yukkuri body) {
		return body.getSameDirectionFactor() * body.getStepBase()[body.getAgeState().ordinal()];
	}
	...
	private static void applyLanding(Yukkuri body) {
		...
		int[] jumpLevel = { 2, 2, 1 };
		int damageCut = 1;
		if (body.getFalldownDamage() >= 8 / jumpLevel[body.getAgeState().ordinal()]) {
			if (body.checkOnBed()) {
				damageCut = 4;
