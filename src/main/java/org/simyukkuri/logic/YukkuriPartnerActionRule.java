...
public final class YukkuriPartnerActionRule {
	...
	public static boolean handleFoundTarget(Yukkuri body, Yukkuri targetBody, Yukkuri bodyHasOkazari,
		...
		if (!targetBody.isDead() && !targetBody.isRemoved()) {
			...
			if (actionGo != YukkuriLogic.ActionGo.NONE) {
				if (actionGo == YukkuriLogic.ActionGo.GO) {
					// 近づきすぎないように近づく
					body.moveToYukkuri(targetBody, targetBody.getX() + collisionOffsetX * 2, targetBody.getY(), zOffset);
			...
			if (YukkuriFoundAffinityRule.handleFoundAffinity(body, targetBody, collisionOffsetX, zOffset)) {
				return true;
			}
		...
		// 死体相手の行動
		return YukkuriDeadSearchRule.handleDeadFound(body, targetBody, collisionOffsetX, zOffset);
	}
}
