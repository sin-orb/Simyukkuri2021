	...
	public class MyMouseListener extends MouseAdapter {
						...
						switch(grabbedObj.getObjType()) {
							case YUKKURI:
								...
								if(b.isPullAndPush() && !b.isDead()) {
									b.wakeup();
									if(b.getZ() <= 0) b.lockSetZ(newZ * Translate.mapZ / Translate.canvasH);
								} else {
									hitX = 4;
		...
		public void mouseMoved(MouseEvent e) {
			...
			synchronized(lock) {
				...
				// フィールドツール,ベルトコンベアの場合
				if( (sel.getGroup() != MainCategoryName.BARRIER && sel.getGroup() != MainCategoryName.CONVEYOR) || sel.getInitOption() == 0) return;
				if(fieldSX < 0 || fieldSY < 0) return;
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			...
			if(select >= MainCommandUI.gameSpeedCombo.getItemCount()) select = MainCommandUI.gameSpeedCombo.getItemCount() - 1;
