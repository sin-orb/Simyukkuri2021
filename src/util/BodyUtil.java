	...
	public static void drawBody(Graphics2D g2, ImageObserver io, Body b) {
		...
		// 通常(斜視)
			...
			switch(layer2.option[0]) {
				...
				case 2:
					// すっきり
					jh = Translate.transSize(jumpTable[(int)b.getAge()/2 % 9]/2/BODY_JUMP_LEVEL[ageIndex]*5/2);
					jy = -jh;
					break;
				case 3:
					// 通常ジャンプ
				...
				case 5:
					// ゆんやあーー
					jh = Translate.transSize(jumpTable[(int)b.getAge()/2 % 9]/2/BODY_JUMP_LEVEL[ageIndex]*(-5) );
					jy = -jh;
					break;
			}
			...
			if( oLinkParent != null && oLinkParent.getZ() < b.getZ() && oLinkParent instanceof Body ){
				...
				switch(layer2Link.option[0]) {
					...
					case 1:
						// 発情 大ジャンプ
						jyLink = -Translate.transSize(jumpTable[(int)bLink.getAge() % 9]/BODY_JUMP_LEVEL[bLink.getBodyAgeState().ordinal()]);
						break;
					case 2:
						// すっきり
						jhLink = Translate.transSize(jumpTable[(int)bLink.getAge()/2 % 9]/2/BODY_JUMP_LEVEL[bLink.getBodyAgeState().ordinal()]*5/2);
						jyLink = -jhLink;
						break;
					case 3:
						// 通常ジャンプ
						jyLink = -Translate.transSize(jumpTable[(int)bLink.getAge() % 9]/2/BODY_JUMP_LEVEL[bLink.getBodyAgeState().ordinal()]);
						break;
					case 4:
						// のびのび
						jhLink = Translate.transSize(jumpTable[(int)bLink.getAge()/2 % 9]/2/BODY_JUMP_LEVEL[bLink.getBodyAgeState().ordinal()]*5/2);
			...
			if(b.isBraidType()){
				...
				for(int i = 0; i < numBr; i++) {
					//g2.drawImage(layer.image[i], braid.screenRect[layer.dir[i]].x, braid.screenRect[layer.dir[i]].y + faceOfsY, braid.screenRect[layer.dir[i]].width, braid.screenRect[layer.dir[i]].height, io);
					drawBody(g2, z, faceOfsY, layer3.image[i], braid.screenRect[layer3.dir[i]].x,	braid.screenRect[layer3.dir[i]].y + faceOfsY,
			...
			if(numAB!=0){
				for(int i= 0 ; i<numAB ; i++){
					drawBody(g2, z, (expand.screenRect[layer.dir[i]].height / 4), layer.image[i],
							expand.screenRect[layer.dir[i]].x, expand.screenRect[layer.dir[i]].y + (expand.screenRect[layer.dir[i]].height / 4),
							expand.screenRect[layer.dir[i]].width, expand.screenRect[layer.dir[i]].height,
							base.screenRect[layer.dir[i]].width, base.screenRect[layer.dir[i]].height, io);
				}
			}
				...
				for(int i = 0; i < num2; i++) {
					drawBody(g2, z, faceOfsY, layer2.image[i], base.screenRect[layer2.dir[i]].x, base.screenRect[layer2.dir[i]].y + faceOfsY,
			...
			if(b.getOkazari() != null) {
				...
				if(otype == OkazariType.DEFAULT) {
					...
					int ofsX = Translate.transSize(ofs.x);
					int ofsY = Translate.transSize(ofs.y);
