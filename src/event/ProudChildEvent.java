...
public class ProudChildEvent extends EventPacket {
	...
	public boolean checkEventResponse(Body body) {
		...
		body.setWorldEventResMessage(GameMessages.getMessage(body, MessagePool.Action.ProudChildsGO), Const.HOLDMESSAGE, true,
				false);
	...
	public UpdateState update(Body body) {
		...
		if ((body.isFirstGround() || body.isNewborn() || body.getZ() > body.getMostDepth()) && !body.isPartner(sourceBody)) {
			return UpdateState.ABORT;
		}
		...
		if (body == sourceBody) {
			...
			switch (state) {
				...
				case START:
					if (checkWait(body, waitTicks)) {
						if (!childActionEnabled) {
							body.setBodyEventResMessage(GameMessages.getMessage(body, MessagePool.Action.ProudChildsSTARTFrom),
									52,
									true, false);
				...
				case SING:
					if (checkWait(body, waitTicks)) {
						if (!childActionEnabled) {
							body.setBodyEventResMessage(GameMessages.getMessage(body, MessagePool.Action.ProudChildsSING), 52,
									true, false);
				...
				case PROUD:
					if (checkWait(body, waitTicks)) {
						if (!childActionEnabled) {
							body.setBodyEventResMessage(GameMessages.getMessage(body, MessagePool.Action.ProudChildsPROUDFrom),
									52,
									true, false);
				...
				case END:
					if (!childActionEnabled) {
						body.setBodyEventResMessage(GameMessages.getMessage(body, MessagePool.Action.ProudChildsENDFrom),
								52,
								true, false);
						childActionEnabled = true;
						body.stay(52);
			...
			switch (state) {
				...
				case START:
					if (childActionEnabled) {
						if (checkWait(body, waitTicks)) {
							body.setBodyEventResMessage(GameMessages.getMessage(body, MessagePool.Action.ProudChildsSTART), 52,
									true, false);
							body.stay(stayTicks);
							body.addMemories(10);
						}
				...
				case PROUD:
					if (childActionEnabled) {
						if (checkWait(body, waitTicks)) {
							body.setBodyEventResMessage(GameMessages.getMessage(body, MessagePool.Action.ProudChildsPROUD), 52,
									true, false);
				...
				case END:
			if (body.isRude())
				body.setBodyEventResMessage(GameMessages.getMessage(body, MessagePool.Action.ProudChildsEND), 52, true,
								false);
			body.stay(52);
			return UpdateState.ABORT;
				default:
