...
public class YukkuriLogic {
	...
	public static final boolean checkPartner(Yukkuri body) {
		if (YukkuriPartnerEntryRule.shouldSkipPartnerAction(body)) {
			return false;
		}
		...
		Yukkuri mappedTarget = YukkuriPartnerEntryRule.resolveMappedTarget(body, nearestDistance);
		...
		Yukkuri bodyOldMoveTarget = YukkuriPartnerEntryRule.resolveMoveTarget(body);

		// 発情時
		// レイパーですっきり中なら続けて同ターゲットに
		Yukkuri partner = YukkuriPartnerEntryRule.getPartnerIfPreferred(body);
		...
		// 自分が泣き叫んでいるなら、他ゆに目をくれない。
			...
			if (targetBody == null) {
				YukkuriPartnerSearchRule.SearchResult searchResult = YukkuriPartnerSearchRule.selectTargets(body, targetBody,
						nearestDistance, secondNearestDistance);
				targetBody = searchResult.getTargetBody();
				bodyHasOkazari = searchResult.getTargetHasOkazari();
			}
		}

		// 目標が定まっていないなら終了
		if (targetBody == null) {
			if (YukkuriPartnerEntryRule.handleNoFoundTarget(body)) {
				return true;
			}
			return handled;
		}
		...
		return YukkuriPartnerActionRule.handleFoundTarget(body, targetBody, bodyHasOkazari, colX, mz);
	}
	...
	public static final boolean doActionOther(Yukkuri self, Yukkuri target) {
		...
		if (YukkuriContactRule.handleAdjacentContact(self, target, collisionOffsetX, distanceY, collisionRange)) {
			return true;
		}
		if (target.isToSteal()) {
			return false;
	...
	public static final ActionGo checkActionSurisuriFromPlayer(Yukkuri body, Yukkuri bodyTarget) {
		return YukkuriSurisuriRule.checkActionSurisuriFromPlayer(body, bodyTarget);
	}
	...
	public static final void gatheringYukkuri() {
		YukkuriGatheringRule.gatheringYukkuri();
	}
	...
	public static final boolean gatheringYukkuriFront(Yukkuri topBody, List<Yukkuri> targetList) {
		return YukkuriGatheringRule.gatheringYukkuriFront(topBody, targetList);
	}
	...
	public static final boolean gatheringYukkuriFront(Yukkuri topBody, List<Yukkuri> targetList, EventPacket event) {
		return YukkuriGatheringRule.gatheringYukkuriFront(topBody, targetList, event);
	}
	...
	public static final boolean gatheringYukkuriSquare(Entity topObject, Yukkuri[] targetList,
			...
			EventPacket event) {
		return YukkuriGatheringRule.gatheringYukkuriSquare(topObject, targetList, direction, event);
	}
	...
	public static final boolean gatheringYukkuriBackLine(Yukkuri topBody, List<Yukkuri> targetList, EventPacket event) {
		return YukkuriGatheringRule.gatheringYukkuriBackLine(topBody, targetList, event);
	}
	...
	public static boolean checkEmotionFromUnunSlave(Yukkuri body, Yukkuri bodyTarget) {
		return YukkuriUnunSlaveEmotionRule.checkEmotionFromUnunSlave(body, bodyTarget);
	}
	...
	public static void checkNearParent(Yukkuri body) {
		YukkuriParentRule.checkNearParent(body);
	}
	...
	public static void checkNearParent(SocialEntity body) {
		YukkuriParentRule.checkNearParent((Yukkuri) body);
	}
	...
	public static boolean checkWakeupOtherYukkuri(Yukkuri body) {
		return YukkuriWakeupRule.checkWakeupOtherYukkuri(body);
	}
}
