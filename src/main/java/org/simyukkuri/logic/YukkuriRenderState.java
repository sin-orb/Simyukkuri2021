 ...
 * </p>
 */
public final class YukkuriRenderState {
	private YukkuriRenderState() {
	}
	 ...
	 * @return 追加した画像数
	 */
	public static int getDamageImageIndex(Yukkuri body, YukkuriLayer layer) {
	 ...
	 * @return 追加した画像数
	 */
	public static int getEffectImage(Yukkuri body, YukkuriLayer layer) {
		int direction = body.getDirection().ordinal();
		int idx = 0;
	 ...
	 * @return 追加した画像数
	 */
	public static int getOkazariImageIndex(Yukkuri body, YukkuriLayer layer, int type) {
		int direction = body.getDirection().ordinal();
		int idx = 0;
	 ...
	 * @return 追加した画像数
	 */
	public static int getBraidImage(Yukkuri body, YukkuriLayer layer, int type) {
		int direction = body.getDirection().ordinal();
		int idx = 0;
	...
	private static void applyFaceJumpOption(Yukkuri body, YukkuriLayer layer) {
	...
	private static int appendSleepingFace(Yukkuri body, int direction, YukkuriLayer layer, int idx) {
		if (!SimYukkuri.UNYO) {
	...
	private static int appendEatingFace(Yukkuri body, int direction, YukkuriLayer layer, int idx) {
	...
	private static int appendDamagedFace(Yukkuri body, int direction, YukkuriLayer layer, int idx) {
	...
	private static int appendNormalStateFace(Yukkuri body, int direction, YukkuriLayer layer, int idx) {
	...
	private static int appendTiredFace(Yukkuri body, int direction, YukkuriLayer layer, int idx) {
		return appendBlinkingFace(body, direction, layer, idx, ImageCode.TIRED.ordinal(), ImageCode.TIRED0.ordinal());
	}

	private static int appendBlinkingFace(Yukkuri body, int direction, YukkuriLayer layer, int idx, int baseFace,
			int blinkBaseFace) {
	...
	private static int appendFaceOverlays(Yukkuri body, int direction, YukkuriLayer layer, int idx) {
		if (body.isShutmouth()) {
	...
	private static int appendFace(Yukkuri body, int imageCode, int direction, YukkuriLayer layer, int idx) {
		return body.getImage(imageCode, direction, layer, idx);
	}

	private static int appendCrushedBody(Yukkuri body, YukkuriLayer layer, int idx) {
		if (body.isBurned()) {
	...
	private static int appendPackedBody(Yukkuri body, YukkuriLayer layer, int idx) {
		if (body.isDead()) {
	...
	private static int appendFrontBody(Yukkuri body, YukkuriLayer layer, int idx) {
	...
	private static int appendRollingBody(Yukkuri body, YukkuriLayer layer, int idx) {
		if (body.getAge() % 8 <= 3) {
	...
	private static int appendRollingStateOverlays(Yukkuri body, YukkuriLayer layer, int idx, boolean left) {
		if (body.getHairState() == HairState.DEFAULT) {
	...
	private static int appendNormalBody(Yukkuri body, int direction, YukkuriLayer layer, int idx) {
		if (body.isPealed()) {
	...
	private static int appendDamageEffect(Yukkuri body, int direction, YukkuriLayer layer, int idx) {
		if (body.isPealed()) {
			return 0;
		}
		...
		if (body.getRank() == YukkuriRank.NORAYU || body.getRank() == YukkuriRank.YASEIYU) {
			return appendFace(body, ImageCode.DAMAGED0.ordinal(), direction, layer, idx);
		}
		return 0;
	}

	private static int appendSickEffect(Yukkuri body, int direction, YukkuriLayer layer, int idx) {
