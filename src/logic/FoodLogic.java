...
/*		// 他の用事がある場合
		...
		if(!b.isVeryHungry()){
			if(b.isToBody() || b.isToBed() || (b.isToShit() && b.getIntelligence() == Intelligence.WISE) || ( b.isAdult() && b.isToSukkiri()) || b.isToSteal() 
		...
		if(ev != null && ev.getPriority() != EventPacket.EventPriority.LOW) {
			if( ev instanceof SuperEatingTimeEvent && ((SuperEatingTimeEvent)ev).getState() == SuperEatingTimeEvent.STATE.START)
			{
				// 食べに行く
				bForceEat = true;
			}
		...
		if( b.isToFood() && food != null) {
				...
				if( food instanceof Food)
				{
					...
					if( f != null )
					{
						if( f.getFoodType() != Food.FoodType.SWEETS1 &&	f.getFoodType() != Food.FoodType.SWEETS2 &&
							f.getFoodType() != Food.FoodType.SWEETS_NORA1 && f.getFoodType() != Food.FoodType.SWEETS_NORA2 &&
							f.getFoodType() != Food.FoodType.SWEETS_YASEI1 && f.getFoodType() != Food.FoodType.SWEETS_YASEI2 )
						{
							return false;
						}
					}
				...
				if (!b.isTalking()) {
					...
					if (food instanceof Food) {
						...
						if( !b.isToTakeout() || b.isVeryHungry()){
							...
							// 食べ切ったら消滅
							if(f.getFoodType() == FoodType.STALK && f.isEmpty()) f.remove();
							// あまあま
							...
							if( f.getFoodType() != FoodType.STALK &&
								f.getFoodType() != FoodType.BITTER && f.getFoodType() != FoodType.HOT && f.getFoodType() != FoodType.WASTE &&
								f.getFoodType() != FoodType.BITTER_NORA && f.getFoodType() != FoodType.HOT_NORA && f.getFoodType() != FoodType.WASTE_NORA &&
								f.getFoodType() != FoodType.BITTER_YASEI && f.getFoodType() != FoodType.HOT_YASEI && f.getFoodType() != FoodType.WASTE_YASEI ||
					...
					//ゆっくり食い
						...
						if(!f.isDead()) {
							...
							if(b.isPredatorType() && !Terrarium.predatorSteam) {
								...
								if(b.canflyCheck() && b.getBodyAgeState().ordinal() > f.getBodyAgeState().ordinal()) {
									...
									if (f.isSick()) b.addSickPeriod(100);
								}
							}
							else {
								// レイパーなら実ゆは食べる
						...
						if (f.isSick() && rnd.nextBoolean()) b.forceSetSick();
						}
					}
					//茎食べ
						...
						if( s.getZ() == 0 && p == null ){
							...
							if( p != null ){
								...
								if( p.getBaryState() == BaryInUGState.ALL ||
								   (p.getBaryState() == BaryInUGState.NEARLY_ALL && !p.hasOkazari()))
								{
									b.setMessage(MessagePool.getMessage(b, MessagePool.Action.FindVegetable), fullmessage);
			...
			//餌に未到着の時
				...
				if(b.canflyCheck()) b.moveTo(food.getX(), food.getY(), 0);
				else b.moveTo(food.getX(), food.getY(), food.getZ());
			}
			return true;
		}
			...
			if (b.isHungry() || forceEat[0] || b.isToTakeout() ) {
				if (!b.isTalking()) {
					...
					if (found instanceof Food) {
						if( b.geteCoreAnkoState() == CoreAnkoState.DEFAULT ){
							//見つけたのがあまあまの時
							if( ((Food)found).getFoodType() == FoodType.SWEETS1 || ((Food)found).getFoodType() == FoodType.SWEETS2 ||
								((Food)found).getFoodType() == FoodType.SWEETS_NORA1 || ((Food)found).getFoodType() == FoodType.SWEETS_NORA2 ||
								((Food)found).getFoodType() == FoodType.SWEETS_YASEI1 || ((Food)found).getFoodType() == FoodType.SWEETS_YASEI2 ) {
	...
	private static final Obj searchFoodNearlest(Body b, boolean[] forceEat) {
		...
		for (Food f: foodList) {
			...
			if (minDistance > distance) {
				if (Barrier.acrossBarrier(b.getX(), b.getY(), f.getX(), f.getY(), Barrier.MAP_BODY[wallMode] + Barrier.BARRIER_KEKKAI)) {
					continue;
				}
				found = f;
		...
		for (ObjEX s :stalkList){
			...
			if (minDistance > distance) {
				if (Barrier.acrossBarrier(b.getX(), b.getY(), s.getX(), s.getY(), Barrier.MAP_BODY[wallMode] + Barrier.BARRIER_KEKKAI)) {
					continue;
				}
				found = s;
				minDistance = distance;
		...
		for (Vomit v: vomitList) {
			...
			if (minDistance > distance) {
				if (Barrier.acrossBarrier(b.getX(), b.getY(), v.getX(), v.getY(), Barrier.MAP_BODY[wallMode] + Barrier.BARRIER_KEKKAI)) {
					continue;
				}
				found = v;
				minDistance = distance;
		...
		for (Body d: bodyList) {
			if (b == d) continue;
			if (!checkCanEatBody(b,d	)) continue;
			...
			if (minDistance > distance) {
				...
				switch(f.getFoodType()) {
					...
					case STALK:
						...
						// ゲスで空腹なら食べる
						else if(b.isRude() && b.isSoHungry()) flag = true;
						// 普通でもとても空腹なら食べる
						else if(!b.isRude() && b.isVeryHungry()) flag = true;
						// レイパーなら食べる
						else if(b.isRaper() && b.isExciting()) flag = true;
						break;
					// あまあま
					case SWEETS1:
					case SWEETS2:
					...
					case WASTE_YASEI:
						// 飢餓状態なら食べる
						if(b.getTangType() == TangType.GOURMET && b.isStarving()) flag = true;
						else if(b.getTangType() ==TangType.NORMAL && b.isTooHungry()) flag = true;
...
//							flagtakeout = true;
	...
	public static final Obj searchFoodPredetor(Body b, boolean[] forceEat) {
		...
		for (Body d: bodyList) {
			...
			if(b.getIntelligence() != Intelligence.FOOL && b.findSick(d) && !b.isTooHungry()) continue;
			//生餌
			if(!d.isDead()) {
				// 捕食種は食べない
			...
			if (minDistance > distance) {
				...
				switch(f.getFoodType()) {
					...
					case STALK:
						...
						// レイパーなら食べる
						else if(!b.isRude() && b.isRaper()) flag = true;
						break;
					// あまあま
					case SWEETS1:
					case SWEETS2:
...
//		int looks = -1000;
		...
		// うんうん奴隷ではない場合
		if( b.getPublicRank() != PublicRank.UnunSlave ) return null;
		...
		if (b.isOnlyAmaama()) {
			switch(foodType) {
				...
				default :
					...
					SimYukkuri.mypane.terrarium.addVomit(b.getX() + 7 - rnd.nextInt(14), b.getY() + 7 - rnd.nextInt(14), 0, b, b.getShitType());
					return;
			}
		}
	...
	private static final void gourmetEating(Body b, FoodType type) {
		...
		case LEMONPOP_NORA:
		case LEMONPOP_YASEI:
			if(b.isRude())b.setHappiness(Happiness.VERY_SAD);
			else b.setHappiness(Happiness.SAD);
			b.setMessage(MessagePool.getMessage(b, MessagePool.Action.EatingBadtasting));
			b.forceToSleep();
			b.addStress(10);
			b.addTang(-20);
	...
	public static boolean checkCanEatBody( Body b, Body p ){
		if(b.isPredatorType())return true;
		if(!p.isDead()) return false;
		if(p.isbindStalk()) return false;
		if(!b.isVeryRude()&& p.hasOkazari()) return false;
		//バカかよっぽどの飢餓状態じゃなきゃかびゆは食べない
		if(b.getIntelligence() != Intelligence.FOOL && b.findSick(p) && !b.isTooHungry()) return false;
		return true;
	}
