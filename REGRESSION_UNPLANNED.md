...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| BITTER食後に excretionBoost が加算されない | `isLikeBitterFood=false`（デフォルト）、ConstState(0)、`FoodLogic.eatFood(body, BITTER, 100)` | `body.getExcretionBoost() == 5`（rapidShit が呼ばれた） | 未着手 |
| BITTER好きなのに rapidShit が走る | `body.setLikeBitterFood(true)`、ConstState(0)、同上 | `body.getExcretionBoost() == 0`（好き食いルートに入り rapidShit なし） | 未着手 |
| excretionBoost があっても shit 蓄積が加速しない | `body.setExcretionBoost(5)`、`body.setAge(1)`（age%100≠0）、`body.checkShit()` | `body.getShit() == 101`（= TICK + 5×20。通常 1/tick が 101/tick に加速） | 未着手 |
| overflow 後に excretionBoost が減衰しない | `body.setExcretionBoost(5)`、`body.setShit(2400)`、`body.setAge(1)`、`body.checkShit()` | `body.getExcretionBoost() == 4`（-1 で減衰）かつ `body.getShit() == 0`（排泄済み） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| NYD中にあまあまが視野内にあっても食べに行かない | `setCoreAnkoState(NON_YUKKURI_DISEASE)` + SWEETS1 を EYESIGHT 範囲内に配置 + `checkFood(body)` | `true`（あまあまサーチが通る）**← 現状 false／実装待ち** | 未着手 |
| NYD中に非あまあまが近くにあるのに食べに行く | 同 NYD 設定 + BITTER を EYESIGHT 範囲内に配置 + `checkFood(body)` | `false`（BITTER は引き続きブロック） | 未着手 |
| あまあまを食べてもストレスが下がらない | `setCoreAnkoState(NON_YUKKURI_DISEASE)` + `FoodLogic.eatFood(body, SWEETS1, 100)` | `body.getStress() == 0` | 未着手 |
| ストレスが下がっても NYD が治癒しない | stress=0 の NYD body + `new YukkuriNydDelegate(body).hasNonYukkuriDisease()` を呼ぶ | `body.isNotNyd() == true`（NORMAL 復帰） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 抗菌スチーム中にカビが進行する | `setDirty(true)` + `addDamage(1)` (isDamaged=true) + `isAntifungalSteam=true` + `checkSick()` | `body.getDirtyPeriod() == 0`（advanceDirtyPeriod が呼ばれない） | 未着手 |
| 抗菌スチームなしでカビが進行しない | 同条件で `isAntifungalSteam=false` | `body.getDirtyPeriod() > 0`（対照確認） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 加湿スチーム中にカビ進行速度が変わらない | `isSick=true` + `addDamage(1)` + `isHumid=true` + `checkSick()` | `body.getSickPeriod() == 4`（通常 1 が 4倍になる） | 未着手 |
| 加湿がないのにカビが加速する | 同条件で `isHumid=false` | `body.getSickPeriod() == 1`（対照確認） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| オレンジスチーム中にダメージが回復しない | `addDamage(500)` + `isOrangeSteam=true` + `checkDamage()` 1回 | `body.getDamage() == 450`（-= TICK*50） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 毒スチーム中にダメージが増えない | `isPoisonSteam=true` + `checkDamage()` 1回 | `body.getDamage() == 100`（+= TICK*100）、`body.isExciting() == false` | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 砂糖スチーム中に重傷ゆっくりが回復しない | `setDamage(limit * 80/100)` (80%以上) + `isSugerSteam=true` + `checkDamage()` 1回 | `body.getDamage() < limit * 80/100`（-= TICK*100） | 未着手 |
| 砂糖スチームが軽傷ゆっくりにも効く | `setDamage(1)` (80%未満) + `isSugerSteam=true` + `checkDamage()` | `body.getDamage()` が 1 以下のまま（80%未満はガードで回復しない） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 眠眠スチーム中にゆっくりが眠ったまま | `setSleeping(true)` + `isNoSleepSteam=true` + `checkSleep()` | `body.isSleeping() == false`、`body.getSleepingPeriod() == 0` | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 加齢促進スチームが効かない | `setAgeState(CHILD)` + `isAgeBoostSteam=true` + `interval=0` + `clockTick()` | `body.getAge()` が 1tick 前より 10001 以上増加（addAge(10000) + 通常 TICK） | 未着手 |
| ADULT にも加齢促進が効く | `setAgeState(ADULT)` + `isAgeBoostSteam=true` + `interval=0` + `clockTick()` | age の 10000 増加なし（ADULT はガード対象） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 加齢停止スチーム中に成長してしまう | CHILD の age を ADULT 閾値-1 に設定 + `isAgeStopSteam=true` + `interval=0` + `clockTick()` | `body.getAgeState() == CHILD`（ADULT に上がらない） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 妊娠促進スチームが効かない | `setHasBaby(true)` + `setPregnantPeriod(getPREGPERIODorg())` + `isRapidPregnantSteam=true` + `clockTick()` | `body.getPregnancyPeriodBoost() > 0`（rapidPregnantPeriod が呼ばれた） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| NYD治癒スチームが効かない | `setCoreAnkoState(NON_YUKKURI_DISEASE)` + `isAntiNonYukkuriDiseaseSteam=true` + `hasNonYukkuriDisease()` | `body.isNotNyd() == true`（即時 NORMAL 復帰） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 捕食抑制スチーム中に捕食種がえさを探す | `setPredatorType(BITE)` + 視野内にゆっくりを配置 + `isPredatorSteam=true` + `checkFood(body)` | `false`（捕食サーチに入らない） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| エンドレスふりふりスチームが効かない | `isEndlessFurifuriSteam=true` + `clockTick()` | `body.isFurifuri() == true`（tickEndlessFurifuri への分岐が発動） | 未着手 |
...
### ANTI_DOS (ドス化防止スチーム) — **未実装・実装必要**

**仕様:** `isAntidosSteam()=true` のとき、まりさが神拳を受けてもドスまりさに変身しない。

**現状:** `Diffuser.SteamType.ANTI_DOS` と `TerrariumEnvironment.antidosSteam` フラグは存在するが、`GameEnvironment.isAntidosSteam()` を読むコードが**どこにもない**。`Marisa.judgeCanTransForGodHand()` に `isAntidosSteam()` チェックを追加する必要がある。

| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| ドス化防止スチーム中にまりさがドス化する | 健全な大人まりさ + `isAntidosSteam=true` + `judgeCanTransForGodHand()` | `false`（変身しない）**← 実装待ち** | 未着手 |
| ドス化防止スチームなしでもドス化しない | 同条件で `isAntidosSteam=false` | `true`（通常は変身可能） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| ゲスでない actor が盗みに成功する | `setAttitude(NICE)`（isRude=false）+ target にお飾りあり + 目撃者なし | `handleOkazariSteal(target, actor)` → `false`、actor にお飾りなし | 未着手 |
| お飾りを持つ actor がさらに盗める | actor に `giveOkazari` でお飾りあり + target にもお飾りあり | `false`（hasOkazari ガード） | 未着手 |
| お飾りのない target から盗める | `target.setOkazari(null)` → hasOkazari=false | `false`（target にお飾りがない） | 未着手 |
| 年齢が違うゆっくりから盗める | `actor.setAgeState(ADULT)`、`target.setAgeState(CHILD)` | `false`（ageState 不一致ガード） | 未着手 |
| 種族が違うゆっくりから盗める | actor=まりさ、target=れいむ（`target.setType(REIMU)` 相当） | `false`（type 不一致ガード） | 未着手 |
| ハイブリッドがお飾りを盗める | `actor.setHybrid(true)` | `false`（isHybrid ガード） | 未着手 |
| DEFAULT 以外のお飾りが盗まれる | target に `giveOkazari(OkazariType.CUSTOM)` 等 DEFAULT 以外を設定 | `false`（OkazariType ガード） | 未着手 |
| ランク条件を満たさない相手から盗める | `target.setPublicRank(KAIYU)` + `actor.setPublicRank(NONE)` | `false`（target が NONE 以外 かつ actor が UNUN_SLAVE 以外） | 未着手 |
| 移動ロック中でも盗める | `actor.setLockmove(true)` | `false`（isLockmove ガード） | 未着手 |
| UNUN_SLAVE が盗み成功してもランクが逆転しない | `actor.setPublicRank(UNUN_SLAVE)`、`target.setPublicRank(NONE)`、actor=ゲス、目撃者なし、`handleOkazariSteal(target, actor)` | `true`、`actor.getPublicRank() == NONE`（解放）、`target.getPublicRank() == UNUN_SLAVE`（降格） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 死んだ赤ゆが food 参照からクリアされない | `setType(BODY.ordinal())`、`setFoods(baby)`、`baby.setDead(true)`、`upDate()` | `feeder.getFoods() == null`、`baby.isRemoved() == true` | 未着手 |
| 成ゆに育って死んでも food がクリアされない | 同上で `baby.setAgeState(ADULT)` + `baby.setDead(true)` | `feeder.getFoods() == null`（AgeState に依存しない仕様の確認） | 未着手 |
| food クリア後に新しい赤ゆが出ない | 上記クリア後に `mode=0` + `upDate()` 再呼び出し | `feeder.getFoods() != null`（headless 不可の場合 assertDoesNotThrow で代替） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 通常種 赤ゆが検出されない | `setting[NORMAL_BABY+0][BABY]=true`、`body.setAgeState(BABY)` | `checkHitObj(body) == true` | 未着手 |
| 通常種 子ゆが検出されない | `setting[NORMAL_BABY+0][CHILD]=true`、`body.setAgeState(CHILD)` | `checkHitObj(body) == true` | 未着手 |
| 捕食種 赤ゆが検出されない | `setting[NORMAL_BABY+1][BABY]=true`、`body.setPredatorType(BITE)` + BABY | `checkHitObj(body) == true` | 未着手 |
| 捕食種 子ゆが検出されない | 同上 + CHILD | `checkHitObj(body) == true` | 未着手 |
| 捕食種 成ゆが検出されない | 同上 + ADULT | `checkHitObj(body) == true` | 未着手 |
| 希少種 赤ゆが検出されない | `setting[NORMAL_BABY+2][BABY]=true`、`body.setRareType(true)` + BABY | `checkHitObj(body) == true` | 未着手 |
| 希少種 子ゆが検出されない | 同上 + CHILD | `checkHitObj(body) == true` | 未着手 |
| 希少種 成ゆが検出されない | 同上 + ADULT | `checkHitObj(body) == true` | 未着手 |
| 足りない種 赤ゆが検出されない | `setting[NORMAL_BABY+3][BABY]=true`、TarinaiReimu（isIdiot=true）+ BABY | `checkHitObj(body) == true` | 未着手 |
| 足りない種 子ゆが検出されない | 同上 + CHILD | `checkHitObj(body) == true` | 未着手 |
| 足りない種 成ゆが検出されない | 同上 + ADULT | `checkHitObj(body) == true` | 未着手 |
| ハイブリッド 赤ゆが検出されない | `setting[NORMAL_BABY+4][BABY]=true`、`body.setHybrid(true)` + BABY | `checkHitObj(body) == true` | 未着手 |
| ハイブリッド 子ゆが検出されない | 同上 + CHILD | `checkHitObj(body) == true` | 未着手 |
| ハイブリッド 成ゆが検出されない | 同上 + ADULT | `checkHitObj(body) == true` | 未着手 |
| うんうんが検出されない | `setting[SHIT][0]=true`、`Shit shit = new Shit()` | `checkHitObj(shit) == true` | 未着手 |
| 吐餡が検出されない | `setting[VOMIT][0]=true`、`Vomit vomit = new Vomit()` | `checkHitObj(vomit) == true` | 未着手 |
| 茎が検出されない | `setting[STALK][0]=true`、`Stalk stalk = new Stalk()` | `checkHitObj(stalk) == true` | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| UP方向で motion が付かない | direction=UP、beltSpeed=MIDDLE | `body.getMotionY() == -2` | 未着手 |
| LEFT方向で motion が付かない | direction=LEFT、beltSpeed=MIDDLE | `body.getMotionX() == -2` | 未着手 |
| BOTTOM方向で motion が付かない | direction=BOTTOM、beltSpeed=MIDDLE | `body.getMotionY() == 2` | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| targetType=0 で Yukkuri の座標が変わらない | `targetType=0`、`cantmove=0`、`beltSpeed=5`、`option=0`、Yukkuri（Y=100） | `yukkuri.getCalcY() == 95`（Y - beltSpeed） | 未着手 |
| targetType=0 で Food の座標が変わらない | 同上で Food（Y=100） | `food.getCalcY() == 95` | 未着手 |
| targetType=0 で Stalk の座標が変わらない | 同上で Stalk | `stalk.getCalcY() == 95` | 未着手 |
| targetType=0 で Vomit の座標が変わらない | 同上で Vomit | `vomit.getCalcY() == 95` | 未着手 |
| targetType=0 で Shit の座標が変わらない | 同上で Shit（Y=100） | `shit.getCalcY() == 95`（既テストは result=0 のみ、座標変化未確認） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| targetType=1（ゆっくりのみ）で Yukkuri が搬送されない | `targetType=1`、`cantmove=0`、`beltSpeed=5`、`option=0`、Yukkuri | `yukkuri.getCalcY()` が変化する | 未着手 |
| targetType=2（うんうん/吐餡）で Shit が搬送されない | `targetType=2`、Shit | `shit.getCalcY()` が変化する | 未着手 |
| targetType=2（うんうん/吐餡）で Vomit が搬送されない | `targetType=2`、Vomit | `vomit.getCalcY()` が変化する | 未着手 |
| targetType=3（食料のみ）で Food が搬送されない | `targetType=3`、Food | `food.getCalcY()` が変化する | 未着手 |
| targetType=4（茎のみ）で Stalk が搬送されない | `targetType=4`、Stalk | `stalk.getCalcY()` が変化する | 未着手 |
| targetType=5（ゆっくり以外）で Food が搬送されない | `targetType=5`、Food | `food.getCalcY()` が変化する | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 死体のみフィルターで生きたゆっくりが搬送される | `filter=true`、`obOptionSelectionList[8]=true`（死体のみ）、alive yukkuri | `return 0` かつ座標変化なし（生ゆは弾かれる） | 未着手 |
| 死体のみフィルターで死亡ゆっくりが弾かれる | 同条件、`body.setDead(true)` | `body.getCalcY()` が変化する（死体は搬送される） | 未着手 |
| 性格フィルターで VERY_NICE が弾かれない | `filter=true`、`obOptionSelectionList[0]=false`（VERY_NICEを除外）、`body.setAttitude(VERY_NICE)` | 座標変化なし（除外される） | 未着手 |
| 知性フィルターで WISE が弾かれない | `filter=true`、`obOptionSelectionList[5]=false`（WISEを除外）、`body.setIntelligence(WISE)` | 座標変化なし（除外される） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 2匹目の dead+crushed が liquidYukkuriType を上書きする | `liquidYukkuriType` を既に設定済み + 別 yukkuri で dead+crushed + `objHitProcess()` | `liquidYukkuriType` が変わらない（1匹目のタイプを維持） | 未着手 |
| crushed でない死亡ゆっくりが液状化する | `body.setDead(true)` + `body.setCrushed(false)` + `liquidYukkuriType=-1` | `liquidYukkuriType == -1` のまま | 未着手 |

### liquidType != -1 のとき baby type への影響

| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 液状化後でも常に母体タイプの赤ゆが生まれる | `setLiquidYukkuriType(SAKUYA.getTypeId())` + `highQuality=true` + `nextInt(50)=0` (hybrid path) + 非ハイブリッド体 | `body.getBabyTypes().get(0).getType()` が母体タイプと異なる（hybridType を返す） | 未着手 |
| 液状化後でも常に母体タイプが選ばれる（liquid パス欠落） | 同上で `nextInt(50)≠0` + `nextBoolean()=true` (liquid path) | `body.getBabyTypes().get(0).getType() == SAKUYA`（liquid タイプそのまま） | 未着手 |
| 液状化後に母体タイプ選択が壊れる | 同上で `nextInt(50)≠0` + `nextBoolean()=false` (mother path) | `body.getBabyTypes().get(0).getType() == body.getType()` | 未着手 |
| ハイブリッド体でも hybrid path に入ってしまう | `body.setHybrid(true)` + `liquidYukkuriType` 設定 + `nextInt(50)=0` | hybrid path に入らない（nextBoolean の結果で liquid or mother type になる） | 未着手 |
| typeId ≥ 10000 でも hybrid path に入ってしまう | `liquidYukkuriType = 10000` + 非ハイブリッド体 + `nextInt(50)=0` | hybrid path に入らない（`liquidYukkuriType < 10000` ガードで弾かれる） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| threshold_medium ちょうどが NONE にならない | `body.setFootBakePeriod(limit >> 1)` | `body.getFootBakeLevel() == FootBake.NONE`（等値は NONE） | 未着手 |
| threshold_critical ちょうどが MEDIUM にならない | `body.setFootBakePeriod(limit)` | `body.getFootBakeLevel() == FootBake.MEDIUM`（等値は MEDIUM） | 未着手 |
| upDate 累積で NONE→MEDIUM に遷移しない | `body.setFootBakePeriod(limit >> 1)`（NONE 上限）、`item.setBoundYukkuri(body)` + `upDate()` 1回（+50 で閾値超え） | `body.getFootBakeLevel() == FootBake.MEDIUM` | 未着手 |
| upDate 累積で MEDIUM→CRITICAL に遷移しない | `body.setFootBakePeriod(limit - 49)`（CRITICAL 手前 50）、同上 + `upDate()` 1回（+50 で `limit+1` に） | `body.getFootBakeLevel() == FootBake.CRITICAL` かつ `body.canPullOrPush() == true` | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 待機中（counter ≤ 60）でも damage が入る | `counter=0`、body を同座標に設定、`upDate()` 1回 | `body.getDamage()` 不変（counter が 1 になるだけ、mixing 未開始） | 未着手 |
| counter ≤ 60 のまま離れると CUT される | `counter=60`（まだ > 60 でない）、body を別座標 → `upDate()` | `body.getCriticalDamageType() != CUT`（60 は > 60 でないのでガード通過） | 未着手 |
| 餡子が尽きても body が除去されない | `counter=60`、`body.setAnkoAmount(100)`、body を同座標、`upDate()` 1回 | `body.isRemoved() == true`（addAmount(-100) → 0 → remove()）かつ `item.getBind() == -1` | 未着手 |
| 死亡後も damage / stress が加算される | `counter=60`、`body.setDead(true)`、body を同座標、`upDate()` 1回 | `body.getDamage()` 不変、`body.getStress()` 不変（isDead ガードで加算スキップ）、`amount += 100` は発生する | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 降ろし後に X 座標が body 位置にならない | shit を (50,50) に作成、body を (200, 100) に移動後 `dropTakeoutItem(SHIT)` | `shit.getCalcX() == 200` | 未着手 |
| 降ろし後に Y 座標が body 位置より +3 にならない（通常パス） | body を worldHeight から十分離れた位置（y+3 ≤ worldHeight）に配置 | `shit.getCalcY() == body.getY() + 3`（**バグ修正後の期待値**。修正前は `body.getY()` を返す） | 未着手 |
| 降ろし後に Z 座標が body.getZ() + 10 にならない | body.getZ() = 0 の状態で drop | `shit.getCalcZ() == 10` | 未着手 |
| 下端付近で Y が +3 にずれる（境界パス） | body.getY() を `worldHeight - 2` に設定（+3 で超過）、drop | `shit.getCalcY() == body.getY()`（**バグ修正後の期待値**。修正前は `body.getY() + 3` を返す） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| 運搬・移動・降ろしを通じて座標が更新されない | shit を (50,50) に設定 → `setCarryItem` → body を (200, 100) に移動 → `dropTakeoutItem` | `shit.getCalcX() == 200`、`shit.getCalcZ() == body.getZ() + 10`（Y は修正後で +3 確認） | 未着手 |
...
| 壊れると | テスト条件 | assert | ステータス |
|---|---|---|---|
| ロード後にうにょが保存時の OFF に戻らない | `world.setUnyo(false)` + `saveState(file)`、その後 `SimYukkuri.UNYO=true` に変更 → `loadState(file)` | `SimYukkuri.UNYO == false`（保存時の値に復元） | 未着手 |
| ロード後にうにょが保存時の ON に戻らない | `world.setUnyo(true)` + `saveState(file)`、その後 `SimYukkuri.UNYO=false` に変更 → `loadState(file)` | `SimYukkuri.UNYO == true` | 未着手 |
| ロード後にワールド幅が保存時の windowType に対応した値にならない | `world.setWindowType(0)` + `world.setTerrariumSizeIndex(1)` + `saveState(file)`、現在の world を `windowType=1` に変更 → `loadState(file)` | `Translate.getWorldWidth() == 300`（windowType=0 の値） | 未着手 |
| ロード後にワールド幅が保存時の terrariumSizeIndex に対応した値にならない | `world.setWindowType(0)` + `world.setTerrariumSizeIndex(2)` (200%) + `saveState(file)`、current を sizeIndex=1 (100%) に変更 → `loadState(file)` | `Translate.getWorldWidth() == 600`（300 × 200/100） | 未着手 |
