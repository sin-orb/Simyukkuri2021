 ...
 */
public final class YukkuriSprite {
	 ...
	 * @param body 対象のゆっくり
	 */
	public YukkuriSprite(Yukkuri body) {
		this.body = body;
	}
	...
	public void changeUnyo(int x, int y, int z) {
		if (!body.isDead() && !body.isCrushed()) {
			...
			if (offsetH > Const.EXT_FORCE_PULL_LIMIT[body.getBodyAgeState().ordinal()]) {
				offsetH = Const.EXT_FORCE_PULL_LIMIT[body.getBodyAgeState().ordinal()];
			} else if (offsetH < Const.EXT_FORCE_PUSH_LIMIT[body.getBodyAgeState().ordinal()]) {
				offsetH = Const.EXT_FORCE_PUSH_LIMIT[body.getBodyAgeState().ordinal()];
			}
			if (offsetW < Const.EXT_FORCE_PUSH_LIMIT[body.getBodyAgeState().ordinal()]) {
				offsetW = Const.EXT_FORCE_PUSH_LIMIT[body.getBodyAgeState().ordinal()];
			} else if (offsetW > Const.EXT_FORCE_PULL_LIMIT[body.getBodyAgeState().ordinal()]) {
				offsetW = Const.EXT_FORCE_PULL_LIMIT[body.getBodyAgeState().ordinal()];
			}
	...
	public void changeReUnyo() {
		...
		int offsetW = body.getUnyoOffsetW();
		if (offsetH == 0) {
		} else if (offsetH < Const.EXT_FORCE_PUSH_LIMIT[body.getBodyAgeState().ordinal()] * 0.6) {
			offsetH += GameRandom.nextInt(3) + 5;
		} else if (offsetH < 0) {
			offsetH += GameRandom.nextInt(3) + 2;
		} else if (offsetH > Const.EXT_FORCE_PULL_LIMIT[body.getBodyAgeState().ordinal()] * 0.6) {
			offsetH -= GameRandom.nextInt(3) + 5;
		} else if (offsetH > 0) {
			offsetH -= GameRandom.nextInt(3) + 2;
		}
		if (offsetW == 0) {
		} else if (offsetW < Const.EXT_FORCE_PUSH_LIMIT[body.getBodyAgeState().ordinal()] * 0.6) {
			offsetW += GameRandom.nextInt(3) + 5;
		} else if (offsetW < 0) {
			offsetW += GameRandom.nextInt(3) + 2;
		} else if (offsetW > Const.EXT_FORCE_PULL_LIMIT[body.getBodyAgeState().ordinal()] * 0.6) {
			offsetW -= GameRandom.nextInt(3) + 5;
		} else if (offsetW > 0) {
			offsetW -= GameRandom.nextInt(3) + 2;
		}
	...
	public int getBodyBaseImage(BodyLayer layer) {
		return BodyRenderState.getBodyBaseImage(body, layer);
	}
	...
	public int getAbnormalBodyImage(BodyLayer layer) {
		return BodyRenderState.getAbnormalBodyImage(body, layer);
	}
	...
	public int getOlazariImage(BodyLayer layer, int type) {
		return BodyRenderState.getOlazariImage(body, layer, type);
	}
	...
	public int getEffectImage(BodyLayer layer) {
		return BodyRenderState.getEffectImage(body, layer);
	}
	...
	public int getFaceImage(BodyLayer layer) {
		return BodyRenderState.getFaceImage(body, layer);
	}
	...
	public int getBraidImage(BodyLayer layer, int type) {
		return BodyRenderState.getBraidImage(body, layer, type);
	}
	...
	public void updateSpriteSize() {
		...
		body.getExpandSpr()[body.getBodyAgeState().ordinal()].addSpriteSize(forceW + expSizeW, forceH + expSizeH);
	}
	...
	public void setBoundary(Dimension4y[] bodyBoundary, Dimension4y[] braidBoundary) {
		int bodyLen = body.getBodySpr().length;
		for (int i = 0; i < bodyLen; i++) {
			...
			body.getBodySpr()[i] = new src.system.Sprite(w, h, src.system.Sprite.PIVOT_CENTER_BOTTOM);
			body.getExpandSpr()[i] = new src.system.Sprite(w, h, src.system.Sprite.PIVOT_CENTER_BOTTOM);
		}
		...
		for (int i = 0; i < braidLen; i++) {
			...
			body.getBraidSpr()[i] = new src.system.Sprite(w, h, src.system.Sprite.PIVOT_CENTER_BOTTOM);
		}
	}
	...
	public void getBoundaryShape(Rectangle r) {
		r.x = body.getBodySpr()[body.getBodyAgeState().ordinal()].getPivotX();
		r.y = body.getBodySpr()[body.getBodyAgeState().ordinal()].getPivotY();
		r.width = body.getBodySpr()[body.getBodyAgeState().ordinal()].getImageW();
		r.height = body.getBodySpr()[body.getBodyAgeState().ordinal()].getImageH();
	}
	...
	public void getExpandShape(Rectangle4y r) {
		r.setWidth(body.getExpandSpr()[body.getBodyAgeState().ordinal()].getScreenRect()[0].getWidth());
		r.setHeight(body.getExpandSpr()[body.getBodyAgeState().ordinal()].getScreenRect()[0].getHeight());
		if (src.SimYukkuri.UNYO) {
			r.setWidth(body.getExpandSpr()[body.getBodyAgeState().ordinal()].getScreenRect()[0].getWidth()
					+ body.getUnyoOffsetW());
			r.setHeight(body.getExpandSpr()[body.getBodyAgeState().ordinal()].getScreenRect()[0].getHeight()
					+ body.getUnyoOffsetH());
	...
	public int getCollisionX() {
		return (body.getBodySpr()[body.getBodyAgeState().ordinal()].getImageW() + body.getExpandSizeW()) >> 1;
	}
	...
	public int getCollisionY() {
		return (body.getBodySpr()[body.getBodyAgeState().ordinal()].getImageH() + body.getExpandSizeH()) >> 1;
	}
	 ...
	 * @return 胴体スプライト
	 */
	public src.system.Sprite getBodyBaseSpr() {
		return body.getBodySpr()[body.getBodyAgeState().ordinal()];
	}
	 ...
	 * @return 拡張スプライト
	 */
	public src.system.Sprite getBodyExpandSpr() {
		return body.getExpandSpr()[body.getBodyAgeState().ordinal()];
	}
	 ...
	 * @return おさげスプライト
	 */
	public src.system.Sprite getBraidSprite() {
		return body.getBraidSpr()[body.getBodyAgeState().ordinal()];
	}
	...
	public BufferedImage getShadowImage() {
		return shadowImages[body.getBodyAgeState().ordinal()];
	}
	...
	public int getShadowH() {
		return shadowImgH[body.getBodyAgeState().ordinal()];
	}
	 ...
	 */
	public int getW() {
		return body.getBodySpr()[body.getBodyAgeState().ordinal()].getImageW();
	}
	 ...
	 */
	public int getH() {
		return body.getBodySpr()[body.getBodyAgeState().ordinal()].getImageH();
	}
	...
	public int getPivotX() {
		return body.getBodySpr()[body.getBodyAgeState().ordinal()].getPivotX();
	}
	...
	public int getPivotY() {
		return body.getBodySpr()[body.getBodyAgeState().ordinal()].getPivotY();
	}
	...
	public int getBraidW() {
		return body.getBraidSpr()[body.getBodyAgeState().ordinal()].getImageW();
	}
	...
	public int getBraidH() {
		return body.getBraidSpr()[body.getBodyAgeState().ordinal()].getImageH();
	}
	...
	public static void setShadowImages(BufferedImage[] shadowImages) {
		YukkuriSprite.shadowImages = shadowImages;
	}
	...
	public static void setShadowImgW(int[] shadowImgW) {
		YukkuriSprite.shadowImgW = shadowImgW;
	}
	...
	public static void setShadowImgH(int[] shadowImgH) {
		YukkuriSprite.shadowImgH = shadowImgH;
	}
	...
	public static void setShadowPivX(int[] shadowPivX) {
		YukkuriSprite.shadowPivX = shadowPivX;
	}
	...
	public static void setShadowPivY(int[] shadowPivY) {
		YukkuriSprite.shadowPivY = shadowPivY;
	}
	...
	public void copyBodySpriteSetFrom(Yukkuri from) {
		...
		body.setBodySpr(copySprites(from.getBodySpr()));
