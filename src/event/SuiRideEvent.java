...
//		boolean ret = false;
		...
		if(!b.canEventResponse() )
		{
			...
			if(b.isParent(getFrom()) || getFrom().isParent(b) || b.isPartner(getFrom()) || getFrom().isSister(b)){
				...
				b.setWorldEventResMessage(MessagePool.getMessage(b, MessagePool.Action.FindGetSuiOtner), Const.HOLDMESSAGE, true, false);
				return true;
			}
		}
				...
				if(s.getcurrent_bindbody_num() >= 3 || tick > 50) {
					...
					if(s.getcurrent_condition() == 1 ){
						...
						if(!memberride || tick%50==0) {
							b.moveTo(rnd.nextInt(Translate.mapW), rnd.nextInt(Translate.mapH - Sui.getBounding().height/2));
						}
						// カウントが500を超える場合
						if(tick > 500) {
							// しゃべってないなら降りる宣言
						...
						if(!b.isTalking()) {
							if(rnd.nextBoolean()) {
								b.setBodyEventResMessage(MessagePool.getMessage(b, MessagePool.Action.RidingSui), Const.HOLDMESSAGE, true, false);
							}
							else{
								b.setBodyEventResMessage(MessagePool.getMessage(b, MessagePool.Action.DrivingSui), Const.HOLDMESSAGE, true, false);
							}
						}
